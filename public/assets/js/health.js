...
function statsFormat(period) {
    switch (period) {
        case 'min':  // Sean: 5min -> min
            return statsFormatMin;
...
function statsFormatMin(time) {  // Sean: 5min -> min
    return new Date(time).toString().match(/(\d+:\d+):/)[1];  // Sean: Use local time instead of UTC
}
